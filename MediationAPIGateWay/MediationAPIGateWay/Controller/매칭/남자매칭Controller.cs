...
namespace MediationAPIGateWay.Controller.매칭
{
    ...
    public class 남자매칭Controller : ControllerBase
    {
        private readonly IMediator _mediator;

        public 남자매칭Controller(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            var command = new Create남자매칭선호Command(dto);
        ...
        // 자리 교환 요청
        [HttpPost("자리교환요청")]
        public async Task<IActionResult> 자리교환요청([FromBody] 자리교환요청Dto dto)
        {
            var command = new Create자리교환요청Command(dto);
            await _mediator.Send(command);
            return Ok("자리 교환 요청이 제출되었습니다.");
        }
    }
}
